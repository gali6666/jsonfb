...
const remoteLog = (message) => {
  ...
  if(!RISK_CODE_CONFIG?.enableRemoteLog) {
    return;
  }
  ...
  const data = buildSignedRequest(
    { message: `[RiskController-${version}] ${message}` },
    { recursive: false }
  );
...
class SandboxManager {
  ...
  createSandboxContext() {
    ...
    const sandboxContext = {
      ...
      // 主进程global
      mainGlobal: globalThis,

      // 配置
      FrontSandboxConfig: RISK_CODE_CONFIG,
      mergeConfig,
      buildConfigJson,
    };
  ...
  async executeCode(codeId, code) {
    try {
      ...
      remoteLog(`Sandbox execution error: ${error.message}`);
      throw error;
    }
  }
  ...
  async executeInit(codeId, code) {
    try {
      ...
      remoteLog(`Init function executed successfully`);
      return result;
    } catch (error) {
      remoteLog(`Init execution error: ${error.message}`);
      throw error;
    }
  }
  ...
  clearCache(codeId) {
    if (codeId) {
      ...
      remoteLog(`Clear cache: ${codeId}`);
    } else {
      this.contextCache.clear();
      remoteLog(`Clear all cache`);
    }
  }
...
async function fetchRemoteRiskCode() {
  ...
  try {
    ...
    const params = buildSignedRequest({ hash: lastRiskCodeHash || '1', type: 'preRisk' });
    ...
    if (
      ...
      try {
        ...
        remoteLog(
          `Polling Code updated successfully, sandbox cache cleared`
        );
        ...
        try {
          ...
          remoteLog(
            `Polling Init function executed after code update`
          );
        } catch (initError) {
          remoteLog(
            `Polling Init function execution failed: ${initError.message}`
          );
        }
        ...
        remoteLog(
          `Polling Code processing failed: ${error.message}`
        );
        return false;
      }
    }
...
function startRiskCodePolling() {
  ...
  // 合并外部配置：读取同目录下的 config.json 并融合进 RISK_CODE_CONFIG。
  // 无论 config.json 是否存在、读取/解析/融合是否成功，都必须静默处理，绝不影响轮询启动。
  try {
    ...
    const externalConfig = JSON.parse(rawConfig);
    if (externalConfig && typeof externalConfig === 'object') {
      mergeConfig(externalConfig);
    }
  } catch (error) {
    // config.json 不存在 / 非法 JSON / 融合失败 一律静默，不阻断流程
    remoteLog(`Failed to merge external config: ${error.message}`);
  }
